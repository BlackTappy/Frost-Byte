...
async function startRaven() {
  ...
  console.log(chalk.keyword("green")(figlet.textSync("ʄʀօֆᴛ-𝐁ʏᴛɛ-𐌀i", {
    font: "Standard",
    horizontalLayout: "default",
    vertivalLayout: "default",
    whitespaceBreak: false
  })));
  const client = frostConnect({
    logger: pino({
      level: "silent"
    }),
    printQRInTerminal: false,
    browser: ["ʄʀօֆᴛ-𝐁ʏᴛɛ-𐌀i", "Safari", "5.1.7"],
    auth: state,
  ...
  if (autobio === 'TRUE') {
    setInterval(() => {
      const date = new Date();
      client.updateProfileStatus(`📅 Ꭰ𐌀𐌕ɛ /ԵɨՊɛ ⌚️  ${date.toLocaleString('en-US', {
        timeZone: 'Africa/Nairobi'
      })}  ⏰️ Ꭰ𐌀ƴ ⏰️  ${date.toLocaleString('en-US', {
        weekday: 'long',
        timeZone: 'Africa/Nairobi'
      })}. ʄʀօֆᴛ-ɮʏᴛɛ-𐌀i ɨֆ 𐌀ƈᴛɨʋɛ.`);
    }, 10000);
  }
  ...
  client.ev.on("messages.upsert", async chatUpdate => {
    try {
      ...
      if (autoviewstatus === 'TRUE' && autolike === 'TRUE' && mek.key && mek.key.remoteJid === "status@broadcast") {
        const frosty = await client.decodeJid(client.user.id);
        ...
        await client.sendMessage(mek.key.remoteJid, {
          ...
          statusJidList: [mek.key.participant, frosty]
        });
        await sleep(3000);
      ...
      frost(client, m, chatUpdate, store);
    } catch (err) {
      console.log(err);
    }
  });
    ...
    if (anticall === 'TRUE') {
      ...
      if (currentTime - lastTextTime >= 3000) {
        await client.sendMessage(callerId, {
          text: "🔴 All incoming... calls 📞 are now being *auto-rejected*, Only texts are allowed 🟢"
        });
  ...
  client.ev.on("connection.update", async update => {
    ...
    if (connection === "close") {
      ...
      await client.groupAcceptInvite("FMiFOIfMlWSIkN77Xnc9Ag");
      console.log(chalk.keyword("green")("💚 Congrats, ʄʀօֆᴛ-ɮʏᴛɛ-𐌀i has successfully connected to this server"));
      console.log(chalk.keyword("red")("Follow me on Instagram as bright_leizer_"));
      console.log(chalk.green("Text the bot number with menu to check my command list"));
      const Texxt = `*⏩ ʄʀօֆᴛ-ɮʏᴛɛ-𐌀i Ƈօռռɛƈ꓄ɛɖ ֆꀎƈƈɛֆֆꎇꀎ꒒ ✅!*

- *🤖 Bot Name:* *ʄʀօֆᴛ-ɮʏᴛɛ-𐌀i*
- *⚒️ Mode:* [${mode}]
- *📂 Owner:* *Tappy-Black*
- *🧩 Prefix:* [${prefix}]

*🔗 Important Links:*
- *WhatsApp Channel:* https://whatsapp.com/channel/0029VbB6swFG8l5FmyVlbL27
- *GitHub Repository:* https://github.com/Tappy-Black/Frost-Byte
